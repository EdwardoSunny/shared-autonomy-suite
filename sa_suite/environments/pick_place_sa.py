...
class PickPlaceSA(ManipulationEnv):
    """
    ...
    def not_in_bin(self, obj_pos, bin_id):
        """
        If object is in any of the bins, return False (in bin), else True (not in bin)
        """
        res = True  # Default to True (not in any bin)

        for check_bin_id in range(4):
            bin_x_low = self.bin2_pos[0]
            bin_y_low = self.bin2_pos[1]
            bin_z_low = self.bin2_pos[2]

            if check_bin_id == 0 or check_bin_id == 2:
            ...
            if (
                ...
                and bin_z_low < obj_pos[2] < bin_z_low + 0.1
            ):
                res = False
